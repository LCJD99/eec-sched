...
class TransformersPipelineRunner(ToolRunner):
    ...
    def __init__(self, task: str, input_port: str, output_port: str, pipeline_factory: Callable[..., Any] | None = None) -> None:
        self.task, self.input_port, self.output_port, self._factory = task, input_port, output_port, pipeline_factory
    ...
    def prepare(self, configuration: Configuration) -> None:
        if configuration.configuration_id not in self._pipelines:
            ...
            self._pipelines[configuration.configuration_id] = factory(self.task, model=model)

    def run(self, inputs: Mapping[str, object], configuration: Configuration) -> Mapping[str, object]:
        result = self._pipelines[configuration.configuration_id](inputs[self.input_port])
...
def representative_tool_specs() -> tuple[ToolSpec, ...]:
    ...
    return (
        ToolSpec("image_caption", "Describe an image.", {"image": Port("image", "image")}, {"text": Port("text", "text")}, (Configuration("base", {"model": "Salesforce/blip-image-captioning-base"}),), True, 0.0),
        ToolSpec("summarize", "Summarize text.", {"text": Port("text", "text")}, {"text": Port("text", "text")}, (Configuration("base", {"model": "sshleifer/distilbart-cnn-12-6"}),), True, 0.0),
...
def register_representative_huggingface_tools(registry: ToolRegistry) -> None:
    ...
    for spec in representative_tool_specs():
        ...
        registry.register(spec, lambda task=task, input_port=input_port, output_port=output_port: TransformersPipelineRunner(task, input_port, output_port))
